...
females and males. It is designed for diploid, biallelic SNP datasets,
including RADseq, DArTseq, and related reduced-representation data. The
function does not alter the GDS. It respects the active sample and
marker selections, restores them before returning, and writes an
auditable results folder.
...
The current `radr` implementation is a redesign rather than an exact
copy of the published `radiator` function. It uses GDS input only, reads
markers in chunks, applies explicit effect-size rules, calculates
method-specific tests and false-discovery rates, audits metadata for
confounding with recorded sex, and produces a separate Y/W panel for
downstream assignment. Cite Devloo-Delva et al. (2024) for the
...
method with different data and model requirements. OutFLANK and pcadapt
are generic outlier methods: they can detect allele-frequency
differences between recorded sexes but do not by themselves establish
sex linkage.
