...
#' \strong{radr} is not re-inventing the wheel here, it uses the
...
#' When RADseq data from DArT is used, \code{\link{explore_genomes}} generates
...
detect_microsatellites <- function(data, gmata.dir = NULL, verbose = TRUE, ...) {
  ...
  tgbase::write_tgbase_tsv(
    ...
    filename = "radr_detect_microsatellites_args",
    date = TRUE,
    internal = FALSE,
  ...
  if (download.gmata) {
    gmata.temp.dir <- file.path(path.folder, "radr_dep")
  ...
  filename <- file.path(
    path.folder,
    stringi::stri_join("radr_detect_microsatellites_", file.date, ".fa")
  )
