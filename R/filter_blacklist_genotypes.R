...
read_blacklist_genotypes <- function(
  ...
  radr.dots <- dotslist[names(dotslist) %in% want]
  erase.genotypes <- radr.dots[["erase.genotypes"]]
  masked.genotypes <- radr.dots[["masked.genotypes"]]
...
#' @param blacklist.genotypes A required blacklist object or filepath containing
#' genotype records to remove or mask.
...
filter_blacklist_genotypes <- function(
  ...
  radr.dots <- dotslist[names(dotslist) %in% want]
  parameters <- radr.dots[["parameters"]]
  erase.genotypes <- radr.dots[["erase.genotypes"]]
  masked.genotypes <- radr.dots[["masked.genotypes"]]
  path.folder <- radr.dots[["path.folder"]]
  ...
  # read whitelist
  blacklist.genotypes <- radr::read_blacklist_genotypes(
  ...
  if (!is.null(blacklist.genotypes)) {
    ...
    if (data.type == "tbl_df") {
      ...
      radr.gds <- gdsfmt::index.gdsn(
        node = data, path = "radr", silent = TRUE)

      id <- gdsfmt::index.gdsn(
        node = radr.gds, path = "individuals", silent = TRUE)
      ...
      wl <- genometranslator::extract_markers_metadata(gds = data, whitelist = TRUE) %$% MARKERS
    }
    ...
    if (n.markers.bl > 0) {
      ...
      if (data.type == "tbl_df") {
        ...
        if (tibble::has_name(blacklist.genotypes, "ALT_DOSAGE")) {
          blacklist.genotypes %<>% dplyr::mutate(ALT_DOSAGE = rep(as.numeric(NA_character_), n()))
        }
        ...
        data  %<>% genometranslator::calibrate_alleles(data = .) %$% input
...
# The section that was in tidy_genome
...
#     }
#
#     if (rlang::has_name(input.erase, "ALT_DOSAGE")) {
#       input.erase <- dplyr::mutate(input.erase, ALT_DOSAGE = rep(as.numeric(NA_character_), n()))
#     }
...
#
#   # required because REF/ALT might change after deleting genotypes...
#   input <- genometranslator::calibrate_alleles(data = input)$input
# } # End erase genotypes
