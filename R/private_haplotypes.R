...
private_haplotypes <- function(data, strata = NULL, verbose = TRUE) {
  # Cleanup-------------------------------------------------------------------
  tgbase::function_header(f.name = "private_haplotypes", verbose = verbose)
  ...
  on.exit(tgbase::function_header(f.name = "private_haplotypes", start = FALSE, verbose = verbose), add = TRUE)
  res <- list()
  ...
  if (!is.null(strata)) {
    ...
    strata$STRATA <- genometranslator::clean_pop_names(strata$STRATA)

    # clean ids
    strata$INDIVIDUALS <- genometranslator::clean_ind_names(strata$INDIVIDUALS)
  ...
  if (!tibble::has_name(data, "HAPLOTYPES")) {
    data <- dplyr::ungroup(data) %>%
      ...
      genometranslator::separate_gt(x = ., gt = "GT_VCF_NUC", exclude = c("MARKERS", "STRATA"), haplotypes = TRUE) %>%
