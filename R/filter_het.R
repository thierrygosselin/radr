...
#'
#' @param strata Optional strata file or object containing individual and group
#' information. Default: \code{strata = NULL}.
...
#'
#' A value of \code{NULL} turns off filtering while retaining the plots and
#' heterozygosity tables.
#' Default: \code{ind.heterozygosity.threshold = NULL}.
...
#' Default: \code{outlier.pop.threshold = 1}.
...
#' @param filename Optional prefix for writing the filtered tidy genomic data as
#' an Arrow Parquet file. The extension \code{.arrow.parquet} is appended.
...
#' \enumerate{
#' \item Inspect individual heterozygosity Manhattan and box plots for unusually
#' high or low samples and compare the pattern with missingness.
#' \item Enter the minimum individual heterozygosity threshold, where 0 turns
#' off the lower filter, followed by the maximum threshold, where 1 turns off
#' the upper filter.
#' \item Choose \code{"haplotype"} or \code{"SNP"}, then choose
#' \code{"overall"} or \code{"pop"}. Haplotype filtering evaluates consistency
#' among SNPs on a locus; SNP filtering treats variants independently.
#' \item Enter \code{het.threshold}. With the haplotype approach, also enter
#' \code{het.dif.threshold}. With the population approach, enter
#' \code{outlier.pop.threshold}, the number of outlier populations tolerated.
#' }
#' The console explains each threshold immediately before requesting it and
#' shows examples of its effect. Use \code{interactive.filter = FALSE} with
...
#'
#' If your a regular radr user, you've seen the \code{pop.num.threshold}.
...
#' heterozygosity in one run with: \pkg{radr} \code{\link{filter_het}}.
...
#' @return A named list containing the filtered tidy data
#' (\code{tidy.filtered.het}), marker whitelist and blacklist, strata,
#' filtering parameters, individual and marker heterozygosity statistics,
#' blacklisted individuals, helper tables, and the generated diagnostic plots.
#' Use \code{names(result)} to inspect all available components. Unlike the
#' newer metadata-only GDS filters, this function currently returns its filtered
#' genomic data in the \code{tidy.filtered.het} component.
#'
#' @examples
#' \dontrun{
...
filter_het <- function(
  ...
  if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
    input <- genometranslator::tidy_genome(
      data = data,
      pop.id = TRUE,
      parallel.core = parallel.core
    )
  } else if (data.type %in% c("tbl_df", "data.frame", "fst.file", "parquet.file")) {
    input <- genometranslator::read_genome(data = data, import.metadata = TRUE)
  } else {
    rlang::abort(paste0(
      "Unsupported input format. Import it first with the appropriate ",
      "genometranslator::read_*() function and supply the resulting GDS object."
    ))
  }
  # create a strata.df
  strata.df <- input %>%
  ...
  # Biallelic data ?
  biallelic <- genometranslator::detect_biallelic_markers(data = input, verbose = FALSE)
...
information")
  ...
  if (tibble::has_name(input, "ALT_DOSAGE")) {
    het.summary <- dplyr::mutate(
      .data = input,
      HET = dplyr::if_else(ALT_DOSAGE == 1, 1, 0, missing = 0)
    ) %>%
      dplyr::group_by(INDIVIDUALS) %>%
      dplyr::mutate(GENOTYPED = length(ALT_DOSAGE[!is.na(ALT_DOSAGE)])) %>%
      dplyr::ungroup(.)
...
number of populations in the dataset turns off the filter.\n")
  ...
  if (!is.null(filename)) {
    ...
    genometranslator::write_genome(data = filter, filename = file.path(path.folder, tidy.name))
  }
