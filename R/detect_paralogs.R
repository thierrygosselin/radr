...
#' \code{\link[radr]{explore_genomes}}.
...
detect_paralogs <- function(
  ...
  tgbase::write_tgbase_tsv(
    data = rad.dots,
    path.folder = path.folder,
    filename = "radr_detect_paralogs_args",
    date = TRUE,
    internal = FALSE,
  ...
  if ("dart" %in% data.source) {
    # individuals.metadata <- genometranslator::extract_individuals_metadata(gds = data, ind.field.select = c("ID_SEQ", "INDIVIDUALS"), whitelist = TRUE)
    markers.metadata <- genometranslator::extract_markers_metadata(
      gds = data,
      markers.meta.select = c("VARIANT_ID", "M_SEQ"),
      whitelist = TRUE
      )
    depth.info <- genometranslator::extract_genotypes_metadata(
      gds = data,
      genotypes.meta.select = c("VARIANT_ID", "M_SEQ", "ID_SEQ", "ALT_DOSAGE", "READ_DEPTH", "ALLELE_REF_DEPTH", "ALLELE_ALT_DEPTH"),
      whitelist = TRUE
    )
    ...
    depth.info %<>%
      ...
      dplyr::select(tidyselect::any_of(c("VARIANT_ID","M_SEQ", "ID_SEQ", "ALT_DOSAGE", "READ_DEPTH", "ALLELE_REF_DEPTH", "ALLELE_ALT_DEPTH")))

  } else {
    depth.info <- genometranslator::tidy_genome(
      data = data,
      ...
      dplyr::select(tidyselect::one_of(c("VARIANT_ID", "M_SEQ", "MARKERS", "STRATA", "INDIVIDUALS", "ALT_DOSAGE")))
