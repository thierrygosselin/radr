...
#'
#' \itemize{
#' \item run \pkg{radr} \code{\link{detect_mixed_genomes}} and
...
#' is not possible...
#' \item use radr::explore_genomes or a more chirurgical approach to
#' coverage and genotype likelihood with radr::explore_genomes.
#' }
#'
#'
...
#' M.A.R., Bender, D., Maller, J., Sklar, P., de Bakker, P.I.W., Daly, M.J. and
...
.filter_hwe_legacy <- function(
  ...
  if (interactive.filter || filter.hwe) {
    ...
    if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
      ...
      gds.bk <- data
      data <- genometranslator::tidy_genome(data = data, pop.id = FALSE, parallel.core = parallel.core)
    ...
    if (run.analysis) {
      ...
      biallelic <- genometranslator::detect_biallelic_markers(data, parallel.core = parallel.core)
      ...
      hwd.helper.table <- tgbase::trans_wide(
      ...
      if (interactive.filter) {
        ...
        do.hw <- tgbase::question(x = hw.q, answer.opt = c("y", "n"))
      } else {
        do.hw <- "y"
      }
      ...
      if (do.hw == "y") {
        ...
        if (interactive.filter) {
          hw.pop.threshold <- tgbase::question(
        ...
        while (no.file) {
          if (interactive.filter) {
            ...
            midp.threshold <- tgbase::question(
        ...
        data <- list.files(
          ...
          pattern = stringi::stri_join("tidy.filtered.hwe.", midp.threshold),
          full.names = TRUE) %>%
          genometranslator::read_genome(data = .)
      }
...
hwe_analysis <- function(x, parallel.core = parallel::detectCores() - 1) {
  hwe_map <- function(x, parallel.core) {
    ...
    hwe_radr <- carrier::crate(function(x, pop = NULL) {
      `%>%` <- magrittr::`%>%`
      `%<>%` <- magrittr::`%<>%`
      mono <- function(x) {
