...
filter_snp_position_read <- function(
  ...
  if (!is.null(filter.snp.position.read) || interactive.filter) {
    ...
    .start <- tgbase::startup(
      package = "radr",
      f.name = "filter_snp_position_read",
      verbose = verbose
    )
    ...
    tgbase::write_tgbase_tsv(
      data = rad.dots,
      path.folder = path.folder,
      filename = "radr_filter_snp_position_read_args",
      date = TRUE,
      internal = internal,
    ...
    if (interactive.filter) {
      ...
      filter.snp.position.read <- tgbase::question(
      ...
      if (filter.snp.position.read == "thresholds") {
        filter.snp.position.read[1] <- tgbase::question(
          x = "Enter the min position of SNP on the read:",
          minmax = c(1,10000))
        filter.snp.position.read[2] <- tgbase::question(
