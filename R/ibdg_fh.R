...
#' These arguments are described in \code{\link[genometranslator]{tidy_genome}}.
#' \itemize{
...
ibdg_fh <- function(
  ...
  # Cleanup-------------------------------------------------------------------
  tgbase::function_header(f.name = "ibdg_fh", verbose = verbose)
  ...
  on.exit(tgbase::function_header(f.name = "ibdg_fh", start = FALSE, verbose = verbose), add = TRUE)

  if (verbose) cat("################################################################################\n")
  if (verbose) cat("############################### radr::ibdg_fh ##############################\n")
  if (verbose) cat("################################################################################\n")
  ...
  # Detect if biallelic --------------------------------------------------------
  biallelic <- genometranslator::detect_biallelic_markers(data)

  # IBDg computations ----------------------------------------------------------
  message("Genome-Wide Identity-By-Descent calculations using FH...")
  if (rlang::has_name(data, "ALT_DOSAGE") && biallelic) {
    # Remove missing
    ...
    freq <- data %>%
      ...
      dplyr::summarise(
        N = n(),
        HOM_ALT = length(ALT_DOSAGE[ALT_DOSAGE == 2]),
        HET = length(ALT_DOSAGE[ALT_DOSAGE == 1])
      ) %>%
      dplyr::mutate(
    ...
    fh <- data %>%
      ...
      dplyr::summarise(
        N = n(),
        HOM_REF = length(ALT_DOSAGE[ALT_DOSAGE == 0]),
        HOM_ALT = length(ALT_DOSAGE[ALT_DOSAGE == 2]),
        HOM = HOM_REF + HOM_ALT
      ) %>%
