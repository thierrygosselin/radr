...
#' Default: \code{allele.threshold = 3}.
#'
...
#'
#'
#' \strong{Input files:} see \pkg{genometranslator} \code{\link[genometranslator]{tidy_genome}}
...
#' \pkg{genometranslator} \code{\link[genometranslator]{tidy_genome}} and
#' \code{\link[radr]{genome_translator}} allows to erase problematic genotypes
...
detect_allele_problems <- function(
  ...
  allele.count <- data %>%
    dplyr::filter(ALT_DOSAGE %in% c(1,2)) %>%
    dplyr::group_by(MARKERS) %>%
    dplyr::summarise(n = sum(ALT_DOSAGE, na.rm = TRUE))
