...
#' \item Remove duplicate \code{\link[radr]{detect_duplicate_genomes}}.
#' \item Remove mixed samples \code{\link[radr]{detect_mixed_genomes}}.
#' \item Look at other filters in radr package...
#' }
#'
...
detect_het_outliers <- function(
  ...
  if (data.type %in% c("tbl_df", "fst.file")) {
    want <- c("MARKERS", "CHROM", "LOCUS", "POS", "INDIVIDUALS", "POP_ID", "REF", "ALT", "GT", "ALT_DOSAGE")
    message("    using tidy data frame of genotypes as input")
    message("    skipping all filters except removal of monomorphic markers")
    res$input <- genometranslator::read_genome(data = data, import.metadata = TRUE) %>%
        dplyr::select(tidyselect::any_of(want))

  # Keeping common markers -----------------------------------------------------
  res$input <- radr::filter_common_markers(data = res$input, verbose = TRUE)


  # Removing monomorphic markers -----------------------------------------------
  res$input <- radr::filter_monomorphic(data = res$input, verbose = TRUE)


} else if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
  res$input <- genometranslator::tidy_genome(
    data = data,
    pop.id = TRUE,
...
biallelic <- genometranslator::detect_biallelic_markers(res$input)
