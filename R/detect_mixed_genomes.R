...
#' @param detect.mixed.genomes (optional, logical) For use inside radr pipelines.
...
#' to use inside \code{\link{explore_genomes}} or when doing population discovery.
...
detect_mixed_genomes <- function(
  ...
  if (interactive.filter || detect.mixed.genomes) {
    ...
    tgbase::write_tgbase_tsv(
      data = rad.dots,
      path.folder = path.folder,
      filename = "radr_detect_mixed_genomes_args",
      date = TRUE,
      internal = internal,
    ...
    if (!data.type %in% c("SeqVarGDSClass", "gds.file")) {
      ...
      if (tibble::has_name(data, "ALT_DOSAGE")) {
        n.markers.pop <- dplyr::filter(data, !is.na(ALT_DOSAGE))
        n.markers.overall <- dplyr::n_distinct(data$MARKERS[!is.na(data$ALT_DOSAGE)])
      } else {
