...
filter_common_markers <- function(
  ...
  if (!filter.common.markers) {
    ...
    tgbase::write_tgbase_tsv(
      data = rad.dots,
      path.folder = path.folder,
      filename = "radr_filter_common_markers_args",
      date = TRUE,
      internal = internal,
    ...
    if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
      ...
      if (n.markers.removed > 0) {
        ...
        blacklist.ncm <- radr::generate_filename(
      ...
      wl <- genometranslator::separate_markers(data = data, sep = "__", markers.meta.lists.only = TRUE)
      ...
      if (tibble::has_name(data, "ALT_DOSAGE")) {
        bl <- dplyr::select(.data = data, MARKERS, STRATA, ALT_DOSAGE) %>%
...
not_common_markers <- function(
  ...
  not_common <- function(
    ...
    parallel.core.opt <- tgbase::parallel_cores(parallel.core)
...
plot_upset <- function(
  ...
  if (data.type == "SeqVarGDSClass") {
    ...
    if (tibble::has_name(x, "ALT_DOSAGE")) {
      plot.data <- dplyr::filter(x, !is.na(ALT_DOSAGE))
    } else {
