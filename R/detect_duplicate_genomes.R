...
#' @param detect.duplicate.genomes (optional, logical) For use inside radr pipelines.
...
#' The default is very fast compared with the genome similarity method. It uses
#' allele counts and codes tailored for biallelic and multiallelic markers.
#' Default: \code{distance.method = "manhattan"}.
...
#'
#' @param dup.threshold Similarity or distance threshold used to identify
#' duplicate genomes. A value of zero turns off filtering.
#' Default: \code{dup.threshold = 0}.
...
#' With the default, the function has no interaction with the user.
#' Default: \code{blacklist.duplicates = FALSE}.
...
detect_duplicate_genomes <- function(
  ...
  if (interactive.filter || detect.duplicate.genomes) {
    ...
    .start <- tgbase::startup(
      package = "radr",
      f.name = "detect_duplicate_genomes",
      verbose = verbose
    )
    ...
    tgbase::write_tgbase_tsv(
      data = rad.dots,
      path.folder = path.folder,
      filename = "radr_detect_duplicate_genomes_args",
      date = TRUE,
      internal = internal,
    ...
    if (!data.type %in% c("SeqVarGDSClass", "gds.file")) {
      tgbase::check_package(package = "amap")

      # Tidy data
      data <- genometranslator::read_genome(data = data, import.metadata = FALSE)
      ...
      if (gt.field == "GT") {
        ...
        geno.stats <- dplyr::filter(data, !is.na(ALT_DOSAGE)) %>%
          dplyr::group_by(INDIVIDUALS) %>%
          dplyr::summarise(GENOTYPED_PROP = length(ALT_DOSAGE) / n.markers)
      }
      readr::write_tsv(
        x = geno.stats,
        file = file.path(path.folder, "genotyped.statistics.tsv"))

      # ALT_DOSAGE available
      # for biallelic data set, just need to keep
      if (!is.null(distance.method) & gt.field == "ALT_DOSAGE") {
        # input.prep <- dplyr::ungroup(data) %>%
        #   dplyr::select(MARKERS, INDIVIDUALS, n = ALT_DOSAGE) %>%
        #   dplyr::mutate(MARKERS_ALLELES = MARKERS) %>%
        #   dplyr::arrange(MARKERS_ALLELES, INDIVIDUALS)

        input.prep <- dplyr::ungroup(data) %>%
          dplyr::select(MARKERS, INDIVIDUALS, ALT = ALT_DOSAGE) %>%
          dplyr::mutate(
            REF = 2 - ALT,
            REF = as.integer(REF),
            ALT = as.integer(ALT)
      ...
      # ALT_DOSAGE NOT available
      if (gt.field != "ALT_DOSAGE") {
        # Allele count
    ...
    if (genome) {
      ...
      if (data.type %in% c("SeqVarGDSClass", "gds.file")) {
        ...
        data <- genometranslator::tidy_genome(
          data = data,
          pop.id = FALSE) %>%
          dplyr::select(INDIVIDUALS, STRATA, MARKERS, ALT_DOSAGE)
      ...
      # If ALT_DOSAGE available, we need a new input.prep (not the same as dist method)



      if (gt.field == "ALT_DOSAGE") {
        input.prep <- dplyr::filter(.data = data, !is.na(ALT_DOSAGE))
      }
      ...
      res$pairwise.genome.similarity <- tgbase::parallel_map(
        .x = all.pairs,
        .f = genome_similarity,
        flat.future = "dfr",
        split.vec = TRUE,
    ...
    if (blacklist.duplicates || interactive.filter) {
      ...
      if (remove.id == "y") {
        ...
        if (remove.dup == 2) {
          ...
          diff.pop.remove <- tgbase::question(
      ...
      if (n.ind.blacklisted > 0) {
        ...
        if (data.type == "tbl_df") {
          ...
          genometranslator::update_genome_gds(gds = data, node.name = "individuals.meta", value = id.info, sync = TRUE)
...
distance_individuals <- function(
  ...
  data.type <- genometranslator::detect_genomic_format(x)
