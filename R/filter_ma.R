...
#' for both alleles. By selecting \code{calibrate.alleles = "depth"}, radr will
...
filter_ma <- function(
  ...
  if (data.type == "tbl_df") {
    wl <- bl <- genometranslator::separate_markers(
      data = data,
      sep = "__",
  ...
  mac.stats <- purrr::map2_dfr(
    .x = mac.data[have],
    .y = have,
    .f = tgbase::tibble_stats
  ) %>%
  ...
  mac.markers.plot <- radr_helper_plot(
  ...
  if (rlang::has_name(mac.data, "MAF_GLOBAL_DEPTH_CORR")) {
    ...
    maf.markers.plot <- radr_helper_plot(
      data = tgbase::trans_long(
  ...
  if (rlang::has_name(mac.data, "MAD_GLOBAL_CORR")) {
    ...
    mad.markers.plot <- radr_helper_plot(
      data = tgbase::trans_long(
...
compute_maf <- carrier::crate(function(x, biallelic) {
  ...
  if (rlang::has_name(x, "ALT_DOSAGE") && biallelic) {
    x %<>%
      dplyr::group_by(MARKERS, POP_ID) %>%
      dplyr::summarise(
        NN = as.numeric(2 * n()),
        PP = as.numeric(2 * length(ALT_DOSAGE[ALT_DOSAGE == 0])),
        PQ = as.numeric(length(ALT_DOSAGE[ALT_DOSAGE == 1])),
        QQ = as.numeric(2 * length(ALT_DOSAGE[ALT_DOSAGE == 2]))
      ) %>%
...
minor_allele_stats <- function(
  ...
  data.type <- genometranslator::detect_genomic_format(data) # gds or tbl_df
  ...
  if (data.type == "SeqVarGDSClass") {
    ...
    if (is.null(markers.meta)) {
      mac.data <- genometranslator::extract_markers_metadata(
        gds = data,
    ...
    if (!is.null(blacklist.markers)) {
      ...
      genometranslator::sync_gds(
        gds = data,
        variant.id = mac.data$VARIANT_ID
      )
    }
    ...
    if (calibrate.alleles == "depth") {
      ...
      ad <- genometranslator::check_coverage(
        gds = data,
      ...
      if ("AD" %in% ad) {
        ...
        calibrate.alleles <- "depth"
        ad <- genometranslator::extract_coverage(
          gds = data,
          individuals = FALSE,
          markers = TRUE,
          coverage = FALSE,
...
ma_one <- carrier::crate(function(x) {
  ...
  # ALT_DOSAGE
  if (rlang::has_name(x, "ALT_DOSAGE")) {
    mac.data <- x %>%
      dplyr::select(MARKERS, INDIVIDUALS, ALT_DOSAGE) %>%
      dplyr::group_by(MARKERS) %>%
      dplyr::summarise(
        PP = as.numeric(2 * length(ALT_DOSAGE[ALT_DOSAGE == 0])),
        PQ = as.numeric(length(ALT_DOSAGE[ALT_DOSAGE == 1])),
  ...
  if (skip && rlang::has_name(x, "GT")) {
    mac.data <- x %>%
      ...
      genometranslator::separate_gt(x = ., gt = "GT", gather = TRUE, exclude = c("MARKERS", "INDIVIDUALS"), split.chunks = 1L) %>%
