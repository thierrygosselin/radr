...
#'
#'
#' This function is used internally in \href{https://github.com/thierrygosselin/radr}{radr}
#' and might be of interest for users.

#' @inheritParams radr_common_arguments


#' @param filter.short.ld (character) Five options:
#' \enumerate{
...
#'
#'
#' \strong{Advanced mode, using \emph{dots-dots-dots}}
#' \itemize{
...
#' (used internally in radr). Default: \code{path.folder = getwd()}.
...
#' \enumerate{
#' \item When loci contain multiple SNPs, choose the SNP-retention rule:
#' \code{1 = mac}, \code{2 = random}, \code{3 = first},
#' \code{4 = middle}, or \code{5 = last}.
#' \item Answer
#' \code{"Do you want to continue filtering using long distance ld? (y/n):"}.
#' Answering no returns after short-distance pruning.
#' \item For reference-guided data, choose whether missingness should determine
#' which SNP is retained from an LD group. De novo data currently uses basic
#' SNPRelate pruning.
#' \item Inspect the LD boxplot and answer
#' \code{"Enter the long LD threshold (filter.long.ld threshold, double/proportion):"}.
#' }
#' Use \code{interactive.filter = FALSE} and explicit short- and long-distance
...
filter_ld <- function(
  ...
  if (is.null(filename)) {
    write.ld <- FALSE
    filename <- stringi::stri_join("radr_", file.date, ".ld")
  } else {
    write.ld <- TRUE
  ...
  if (data.type %in% c("tbl_df", "fst.file")) {
    if (is.vector(data)) data <- genometranslator::read_genome(data = data, import.metadata = FALSE)
    data.type <- "tbl_df"
  ...
  if (interactive.filter || !is.null(filter.short.ld)) {
    ...
    if (any(locus.stats$SNP_N > 1L)) {
      ...
      if (interactive.filter) {
        filter.short.ld <- tgbase::question(
      ...
      if (filter.short.ld == "mac") {
        ...
        if (data.type == "tbl_df") {
          ...
          if (rlang::has_name(data, "ALT_DOSAGE")) {
            ...
            mac.data <- dplyr::filter(data, !is.na(ALT_DOSAGE)) %>%
  ...
  if (interactive.filter || !is.null(filter.long.ld)) {
    ...
    if (interactive.filter) {
      ...
      if (!ref.genome) {
        ...
        long.ld.missing <- tgbase::question(
    ...
    if (data.type == "tbl_df") {# for long distance LD pruning
      ...
      # res.ld$data.gds <- genometranslator::write_gds(
      data.gds <- genometranslator::write_gds(
        data = data,
        filename = filename,
        verbose = FALSE)
    ...
    if (long.ld.missing) {
      ...
      if (interactive.filter) {
        ...
        filter.long.ld <- tgbase::question(
...
ld_boxplot <- function(
  ...
  # R2 is handled in radr, not SNPRelate...
  if (ld.method == "r2") {
    ld.m <- "r"
  } else {
    ld.m <- ld.method
  ...
  ld.res <- 1 - ld.res
  # stats
  ld.summary <- tgbase::tibble_stats(x = ld.res, group = "Long LD")
  ...
  ld.boxplot <- tgbase::boxplot_stats(
    data = ld.summary,
...
ld_missing <- function(
  ...
  if (ld.figures) {
    ...
    ld.summary <- tgbase::tibble_stats(x = fig.data[!is.na(fig.data)],
                               group = "Long LD")
