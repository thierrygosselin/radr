...
#' usually this is automatically generated by \pkg{radr}.
...
detect_biallelic_problems <- function(
  ...
  .start <- tgbase::startup(
    package = "radr",
    f.name = "detect_biallelic_problems",
    verbose = verbose
  )
  ...
  # filename <- stringi::stri_join("radr_biallelic_problem_", format(Sys.time(), "%Y%m%d@%H%M"), ".tsv")
  ...
  if (!tibble::has_name(data, "GT_VCF_NUC")) {
    ...
    message("Tidy dataset requires genotypes with nuclotides: GT_VCF_NUC")
    rlang::abort("One way to get it is to use genometranslator::calibrate_alleles")
  }
  ...
  blacklist.markers <- dplyr::select(data, tidyselect::any_of(want)) %>%
    ...
    genometranslator::separate_gt(x = ., gt = "GT_VCF_NUC", exclude = markers.metadata, split.chunks = 3L, haplotypes = TRUE) %>%
  ...
  if (nrow(blacklist.markers) == 0) {
    ...
    blacklist.info <- dplyr::filter(res$multiallelic.data, GT_VCF_NUC != "./.") %>%
      genometranslator::separate_gt(x = ., gt = "GT_VCF_NUC", exclude = exclude.vars, split.chunks = 3L) %>%
