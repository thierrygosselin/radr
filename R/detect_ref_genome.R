...
#' provided:
#'
#' \strong{1. Radr GDS (preferred)}
#' If a radr-generated GDS is supplied, the function checks, in order:
#' \itemize{
#'   \item the \code{/radr/reference.genome} node (logical value);
#'   \item the \code{/radr/markers.meta/CHROM} field;
#'   \item the \code{SeqArray} \code{chromosome} node.
#' }
#'
#' \strong{2. VCF imported with SeqArray or read by radr}
...
#'
#' @param data
#' A radr GDS object, a \pkg{SeqArray} GDS object, or a path to a file.
#' If provided, the function extracts information from radr metadata,
...
detect_ref_genome <- function(data = NULL, verbose = TRUE) {
  ...
  if (identical(data.type, "dart")) {
    ref.genome <- genometranslator::detect_dart_format(data = data, verbose = FALSE)$ref.genome
  ...
  radr.gds <- gdsfmt::index.gdsn(node = data, path = "radr", silent = TRUE)
  if (is.null(radr.gds)) {
    # no radr node at all -> do not create one, just use SeqArray heuristics
    ...
    if (!is.null(ref.node)) {
      ...
      if (is.logical(ref.val) && length(ref.val) == 1L) {
        # good, scalar logical already stored by radr
        ref.genome <- ref.val
      } else {
        # FUTURE ME:
        # reference.genome node exists but is empty or weird -> recompute
        # and overwrite, but only for files that already have radr structure.
        try.seq.summary <- TRUE
        ref.genome      <- NULL
        sync.gds        <- TRUE
      }
      ...
      # FUTURE ME:
      # reference.genome node is missing inside /radr.
      # We *do not* create it for external / non-radr GDS:
  ...
  if (try.more) {
    ...
    if (is.null(chromosome)) {
      ...
      if (TRUE %in% stringi::stri_detect_fixed(str = data.source, pattern = "GATK")) {
        ...
        chrom.unique.radr <- any(unique(chrom.sample) == "CHROM_1")
        ...
        if (chrom.unique.radr || chrom.unique.stacks) {
          ref.genome <- FALSE
        }
  ...
  # - there is a /radr node, and
  # - we decided earlier that it's safe to overwrite (sync.gds == TRUE).
  if (!is.null(data) && !is.null(radr.gds) && sync.gds) {
...
#'
#' \strong{2. SeqArray / radr GDS files}
...
#'
#' @param data
#' A VCF file path, a radr GDS object, a \pkg{SeqArray} GDS object,
...
extract_ref_genome <- function(data = NULL, verbose = FALSE) {
  ...
  if (identical(data.type, "dart")) {
    ...
    dart.info <- genometranslator::detect_dart_format(data = data, verbose = FALSE)
  ...
  # If it's a file path → read as radr/SeqArray
  if (identical(data.type, "gds.file")) {
    data      <- genometranslator::read_genome(data, verbose = FALSE)
    data.type <- "SeqVarGDSClass"
  }
  ...
  # FUTURE ME:
  # You can store ref file inside /radr/reference.genome.file for radrs,
  # then read it like this:
  #
  # rad <- gdsfmt::index.gdsn(data, "radr", silent = TRUE)
